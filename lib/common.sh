...
function help(){
    ...
    echo ""
    echo "Commands:"
    echo -e $GREEN'create'$COLOR_END    "   Create a new Node. The node hosts Lition, Constellation and Node Manager"
    echo -e $PINK'join'$COLOR_END       "     Create a node and Join to existing Network"
    echo -e $BLUE'attach'$COLOR_END     "   Attach to an existing Lition Node. The node created hosts only Node Manager"
    ...
    echo "  -n, --name              Name of the node to be created"
    echo "  --ip                    IP address of existing Lition"
    echo "  --pk                    Public Key of existing Constellation"
    echo "  -r, --rpc               RPC port of the existing Lition"   
    echo "  -w, --whisper           Discovery port of this node" 
